...
public class ListeLocationsFrame extends JInternalFrame {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private LocationTableModel tableModel = new LocationTableModel();
	...
	protected void showFrame(YapsViewType viewType) {
		...
		if (viewType == FIND) {
			Location location = new Location();
			LocationFrame frame = new LocationFrame(location, FIND);
			desktopPane.add(frame);
			frame.setVisible(true);
			frame.toFront();
			frame.setParentFrame(this);
		...
		for (int selectedRowIndex : selectedRows) {
			Location location = tableModel.getDataList().get(selectedRowIndex);

			if (viewType == READ) {
				LocationFrame frame = new LocationFrame(location, READ);
				desktopPane.add(frame);
				frame.setVisible(true);
				frame.toFront();
			}
			...
			if (viewType == UPDATE) {
				LocationFrame frame = new LocationFrame(location, UPDATE);
				desktopPane.add(frame);
				frame.setVisible(true);
				frame.toFront();
				frame.setParentFrame(this);
			...
			if (viewType == DELETE) {
				LocationCRUD.deleteLocation(location);
				refresh();
			}
		}
	}
