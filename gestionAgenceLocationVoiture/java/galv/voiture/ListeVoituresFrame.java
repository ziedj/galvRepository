...
public class ListeVoituresFrame extends JInternalFrame {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private VoitureTableModel tableModel = new VoitureTableModel();
	...
	protected void showFrame(YapsViewType viewType) {
		...
		if (viewType == FIND) {
			Voiture voiture = new Voiture();
			VoitureFrame frame = new VoitureFrame(voiture, FIND);
			desktopPane.add(frame);
			frame.setVisible(true);
			frame.toFront();
			frame.setParentFrame(this);
		...
		for (int selectedRowIndex : selectedRows) {
			Voiture voiture = tableModel.getDataList().get(selectedRowIndex);

			if (viewType == READ) {
				VoitureFrame frame = new VoitureFrame(voiture, READ);
				desktopPane.add(frame);
				frame.setVisible(true);
				frame.toFront();
			}
			...
			if (viewType == UPDATE) {
				VoitureFrame frame = new VoitureFrame(voiture, UPDATE);
				desktopPane.add(frame);
				frame.setVisible(true);
				frame.toFront();
				frame.setParentFrame(this);
			...
			if (viewType == DELETE) {
				VoitureCRUD.deleteVoiture(voiture);
				refresh();
			}
		}
	}
